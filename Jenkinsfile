...
node {
    ...
    // -------------------------------------------------------------------------
		...
		println 'withCredentials'
            ...
            stage('Deploy') {
                rc = bat returnStdout: true, script: "${toolbelt} sfdx force:mdapi:deploy -d force-app/main/default -x manifest/package1.xml -u ${SF_USERNAME} -w 1000 --testlevel NoTestRun"
                if (rc != 0) {
